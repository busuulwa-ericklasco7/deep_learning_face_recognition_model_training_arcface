# -*- coding: utf-8 -*-
"""Deep Learning Face Recognition Model Training  - ArcFace

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1_SikLoXC-rI4ak5lzW97ZcTmtG9HnMpq

# **ArcFace Training Model For Deep Learning**

*Created by [Erick Busuulwa ](https://x.com/ericklasco) in 2024 at the **Nanjing University of Information Science and Technology** during my masters*


<br>


With this code implementation, anyone can understand the concept of face recognition and train a state-of-the-art(%99.7 LFW Accuracy) facial recogniton model. It will take around 48 hours.
...
PS: Google Colab doesn't support sessions that run longer than 12 hours so you will need to restart it manually after session crashing. But, if it is available in your country, I higgly recommend you to use [Google Colab Pro](https://colab.research.google.com/signup). With pro account, you can run your sessions up to 24 hours and get the best GPU colab has.  
...
<p align="center">🥰 After finishing the training, I got the following results on the test sets as shown below🥰</p>

| Download Link                                                        | Architecture                                                 | Epochs | LFW Acc | AgeDB Acc | CFP Acc |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------ | ------- | --------- | ------- |
| [Google Drive](https://drive.google.com/open?id=1qUtjhrYDMRqc2lRPNBV1ftkbyIPmTJYp) |Resnet50 + ArcFaceLayer                | 7      | %99.70  | %96.75    | %97.34  |

<br>
<br>

- In this gif I tested model on Gilbert Strang and Lex Fridman
...
## The Right GPU
  ...
  - It is better if you train with *Tesla P100*, Colab provides this GPU to PRO users all the time but if you are not PRO, you will need some luck. You re-set the session(**Runtime -> Factory reset runtime**) until you get *Tesla P100*. With ```!nvidia-smi``` command you can see the given GPU 🤖🤖🤖

  - The main reason we need *Tesla P100* is because it has 16GB VRAM 😵
"""
...
## Downloading the Dataset
  ...
  - I recommend **ms1m-arcface** as its what i used but you are welcome to choose another dataset. Visit this [page](https://github.com/aangfanboy/deepface/tree/master/datasets) for more options.
...
"""## UnZipping and Installing Libraries
...
**PS:** *MxNet* will be used for dataset processing, this is an implementation with *TensorFlow*
"""

class ArcFaceLayer(tf.keras.layers.Layer):
    def __init__(self, num_classes, arc_m=0.5, arc_s=64., regularizer_l: float = 5e-4, **kwargs):
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(name="kernel", shape=[512, self.num_classes], initializer='glorot_normal',
                                      regularizer=tf.keras.regularizers.l2(self.regularizer_l), trainable=True)
...
# !pip install mxnet==1.8
# !pip install numpy==1.23.5
!pip install tensorflow-addons
!pip install efficientnet

"""I use TensorFlow 2.x and the command below will load last available version of TensorFlow 2"""
...
- I will use [TFRecord](https://www.tensorflow.org/tutorials/load_data/tfrecord) dataset type, because it is fast 💨💨💨
"""
...
- After downloading code will unzip the dataset and delete the zip file.
"""
...
- We need to mount our google drive to colab page because model checkpoints will be saving to drive 💽💽💽
"""

from google.colab import drive
drive.mount('/gdrive')

"""## Defining ArcFace Layer
...
But simply, that is what ArcFace method does. It creates a gap between inter-classes. This way, model gets better as a discriminator and be the perfect choice for [one shot learning](https://en.wikipedia.org/wiki/One-shot_learning)
"""
...
- We will use TensorBoard to graph the change in loss and accuracy
"""
...
"""## Defining LResNet-x-IR Model Architecture
...
- I  will need a data pipeline to stream face images to model for both training and testing. I will use TFRecord data type and [TensorFlow Dataset Object](https://www.tensorflow.org/api_docs/python/tf/data/Dataset) for this task.
...
"""## Defining a Test Script for LFW

- This code will test our face recognition model on LFW dataset that we downloaded earlier 🧪🧪🧪
"""
...
"""## Defining a Main Trainer

- This is the piece of code that will optimize everything and train the model.

<br>

**PS:** Parameters supported with comments in ```__init__``` function.
"""
...
- I mentioned that we will be using TensorBoard to graph loss and accuracy values. The code below will run TensorBoard and you will be able to see graphs when main trainer starts working.
"""
...
- Cells below will start the training with given parameters. Parameters supported with comments so you are welcome to change. But if you want to get the same results that I did, leave them be.
"""
...
trainer = Trainer(
  ...
  use_arcface=True,
  learning_rate=0.004,
  model_path="/gdrive/My Drive/my_ms1m_arcResnetIR50/model.tf",
  optimizer="SGD",
  lr_step_dict={
...
trainer.save_final_model(path="/gdrive/My Drive/my_ms1m_arcResnetIR50_arcface_final.h5")
